...
Cmeans_SHRmgmt <- rbind(coarse_w_no_res$C_means, till_irr_ann_loamy_w_no_res$C_means, notill_irr_ann_loamy_w_no_res$C_means, notill_irr_per_loamy_w_no_res$C_means, till_dry_ann_loamy_w_no_res$C_means, notill_dry_ann_loamy_w_no_res$C_means, coarse_loamy_w_res$C_means, loamy_w_res$C_means, shrink_swell$C_means)
row.names(Cmeans_SHRmgmt) <- c('1', '2a', '2b', '2c', '2d', '2e', '3', '4', '7')
colnames(Cmeans_SHRmgmt) <- c('0-10 cm', '10-30 cm', '30-50 cm', '50-100 cm')
Cmeans_SHRmgmt
C_SE_SHRmgmt <- rbind(coarse_w_no_res$C_se, till_irr_ann_loamy_w_no_res$C_se, notill_irr_ann_loamy_w_no_res$C_se, notill_irr_per_loamy_w_no_res$C_se, till_dry_ann_loamy_w_no_res$C_se, notill_dry_ann_loamy_w_no_res$C_se, coarse_loamy_w_res$C_se, loamy_w_res$C_se, shrink_swell$C_se)
C_SE_SHRmgmt
C_n_SHRmgmt <- rbind(coarse_w_no_res$n, till_irr_ann_loamy_w_no_res$n, notill_irr_ann_loamy_w_no_res$n, notill_irr_per_loamy_w_no_res$n, till_dry_ann_loamy_w_no_res$n, notill_dry_ann_loamy_w_no_res$n, coarse_loamy_w_res$n, loamy_w_res$n, shrink_swell$n)
C_n_SHRmgmt

tiff(file = file.path(FiguresDir, "Napa Lodi", 'Napa_Lodi_soilC_barchart_alldepths_FINAL.tif'), pointsize = 11, family = 'Times New Roman', width = 9, height = 4.25, units = 'in', res=800, compression = 'lzw')
par(mar=c(2.4, 3.5, 1.25, 0.5))
Cmeans_bardims <- barplot(Cmeans_SHRmgmt, beside=TRUE, legend.text=c('1. Coarse with no restrictions (n=14)', '2a. Loamy with no restrictions: tilled (n=18)', '2b. Loamy with no restrictions: no-till (n=5)', '2c. Loamy with no restrictions: no-till & perennial (n=13)', '2d. Loamy with no restrictions: tilled & dryfarm (n=12)', '2e. Loamy with no restrictions: no-till & dryfarm (n=6)', '3. Low OM & restrictive horizons (n=18)', '4. High OM with restrictive horizons (n=13)', '7. Shrink-swell (n=4)'), col=c('lightgoldenrod', 'tan4', 'tan4', 'tan4',  'tan4', 'tan4', 'olivedrab3', 'firebrick3', 'violetred'), ylab='', ylim=c(0,3.5), args.legend=list(x='topright', cex=1, pt.cex=1, bty='n', ncol=1, inset=c(0,-0.06), x.intersp=0.4), cex.names=1)
