...
cluster_radar_plot <- function(cluster_no) {
  ...
  tiff(file = file.path(FiguresDir, 'v2', paste0('valley_', cluster_no, '_classes_spider_Apr2020.tif')), family = 'Times New Roman', width = 6.5, height = 6.5, pointsize = 11, units = 'in', res=800, compression = 'lzw')
  par(mar=rep(0.1, 4))
...
shapefile(valley_mu_shp_30cm, file.path(dataDir, 'FINAL results', 'shapefiles with data', 'valley_30cm_cluster.shp'), overwrite=TRUE)
...
write.csv(cluster_fk, file.path(dataDir, 'FINAL results', 'clusters2_to_12_df_for_radarchart.csv'), row.names = FALSE)
